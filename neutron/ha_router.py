...
class HaRouter(router.RouterInfo):
    ...
    def refresh_accept_ra(self, accept_ra):
        flag = n_consts.ACCEPT_RA_DISABLED
        if accept_ra:
            flag = n_consts.ACCEPT_RA_WITHOUT_FORWARDING
        for internal_port in self.internal_ports:
            interface_name = self.get_internal_device_name(
                internal_port['id'])
